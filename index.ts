...
export const updatePresence = async clientInstance => {
  ...
  presenceInterval = setInterval(() => {
    ...
    const activities = [
      { name: '⚡ Kenium 4.7.0 ⚡', type: 1, url: 'https://www.youtube.com/watch?v=7aIjwQCEox8' },
...
aqua.on('trackStart', async (player, track) => {
  try {
    ...
    if (now - lastVoiceStatusUpdate > VOICE_STATUS_THROTTLE) {
      ...
      const status = `⭐ ${truncateText(track.info?.title || track.title, VOICE_STATUS_LENGTH)} - Kenium 4.7.0`
