...
const _functions = {
  ...
  extractMacroCalls(body: any) {
    ...
    return {
      lyrics: calls['track.lyrics.get']?.message?.body?.lyrics?.lyrics_body as string | undefined,
      track: calls['matcher.track.get']?.message?.body?.track,
      subtitles: calls['track.subtitles.get']?.message?.body?.subtitle_list?.[0]?.subtitle?.subtitle_body as string | undefined
    }
  }
}
...
export class Musixmatch {
  ...
  async callMxm(endpoint: string, params: Record<string, string | undefined>): Promise<any> {
    try {
  ...
  formatResult(subtitles: string | null, lyrics: string | null, track: any): LyricsResult {
    const lines = subtitles ? this.parseSubtitles(subtitles) : null
    const text = lyrics ? this.cleanLyrics(lyrics) : lines?.map(l => l.line).join('\n') ?? null
  ...
  async raceForFirst<T>(promises: Promise<T | null>[]): Promise<T | null> {
    ...
    return new Promise(resolve => {
      ...
      const settle = (result?: T | null) => {
        ...
        if (result) { resolved = true; return resolve(result) }
