...
function tableExists(rawDb: any, tableName: string): boolean {
    try {
        return !!rawDb.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name=?").get(tableName);
    } catch {
        return false;
    }
}
...
export async function migrateDatabase() {
    ...
    console.log(`[Migration] Starting migration from ${sourceTable} to V2 schema...`);
    ...
    try {
        if (typeof rawDb.backup === 'function') {
            ...
            console.warn("[Migration] Native backup API not found. Performing checkpoint...");
    ...
    const stmt = rawDb.prepare(`SELECT _id, doc, createdAt, updatedAt FROM ${sourceTable}`);
    ...
    db.transaction(() => {
        for (const row of stmt.iterate()) {
            try {
                ...
                playlistsV2.insert({
                    ...
                    totalDuration: doc.totalDuration || tracks.reduce((sum: number, t: any) => sum + (t.duration || 0), 0),
        ...
        if (failureCount === 0 && successCount > 0) {
            console.log(`[Migration] Successfully migrated ${successCount} playlists.`);
            ...
            console.warn(`[Migration] Migration completed with ${failureCount} errors. Source table NOT renamed.`);
        }
    });
