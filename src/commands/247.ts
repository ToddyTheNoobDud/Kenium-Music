  ...
  public override async run(ctx: CommandContext): Promise<void> {
    try {
      ...
      void (async () => {
        try {
          ...
          const targetNick = newEnabled
            ? (baseNick.includes(NICKNAME_SUFFIX) ? baseNick : baseNick + NICKNAME_SUFFIX)
            : baseNick.replace(/ ?\[24\/7\]/, "");

          if (botMember.nick !== targetNick) await botMember.edit({ nick: targetNick });
        } catch {}
      })();
      ...
      const embed = new Embed()
        ...
        .setDescription(
          newEnabled ? (t.mode247?.enabled || "24/7 mode enabled") : (t.mode247?.disabled || "24/7 mode disabled"),
        )
