...
import { ActionRow, StringSelectMenu } from 'seyfert'
...
function createSelectMenu(customId: string, placeholder: string, opts: { label: string; value: string; description?: string; emoji?: string }[]) {
  return new ActionRow().addComponents(
    new StringSelectMenu()
      .setCustomId(customId)
      .setPlaceholder(placeholder)
      // @ts-ignore
      .addOption(opts.map(o => ({
        label: o.label,
        value: o.value,
  ...
  async run(ctx: CommandContext) {
    ...
    if (!playlistName) {
      ...
      playlists.slice(0, 10).forEach(p => {
        ...
        const lastMod = new Date(p.lastModified || p.createdAt).toLocaleDateString()
        embed.addFields({
    ...
    const embed = createEmbed('primary', `${ICONS.playlist} ${playlistName}`, undefined, [
      ...
      { name: `${ICONS.tracks} Tracks`, value: String(playlist.tracks.length), inline: true },
    ...
    if (trackList) embed.addFields({ name: `${ICONS.music} Tracks (Page ${page}/${totalPages})`, value: trackList, inline: false })
